function hand = handPosture( hand, q )
...
% Remove any alignment transforms
if hand.aTflags
    T = inv(hand.aT);
    R = [ T(:,1:3) [0; 0; 0; 1] ];
    ...
    for j = 1:m
        hand.palm.links(j).collisionMesh.vertices           = (h2e( T * e2h(hand.palm.links(j).collisionMesh.vertices.') ))';
    ...
    [~, n] = size(hand.digits); % get number of digits
    for i = 1:n
...
if isfield(hand.palm, 'links') % Check if there are movable parts in the palm
    % Calculate old and new transform for all joints
...
% Reapply any alignment transforms
if hand.aTflags
    T = hand.aT;
    R = [ T(:,1:3) [0; 0; 0; 1] ];
    hand.palm.root.collisionMesh.vertices           = (h2e( T * e2h(hand.palm.root.collisionMesh.vertices.') ))';
