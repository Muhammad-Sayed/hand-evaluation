function [ object, range, anthroReference ] = grasp10_PowerDisk( hand )
...
% Import object; MiniCD
[object.faces, object.vertices, object.normals] = stlread('objectModels\Cylinder_80dX2l_80faces_miniCD.stl');

% Calculate object pose range
%   Transition;
%   X
range(1,1) = max(hand.palm.root.collisionMesh.vertices(:,1)) - 60;
range(1,2) = max(hand.palm.root.collisionMesh.vertices(:,1)) - 20;
%   Y
range(2,1) = min(hand.palm.root.collisionMesh.vertices(:,2));
range(2,2) = max(hand.palm.root.collisionMesh.vertices(:,2));
%   Z
range(3,1) = min(hand.palm.root.collisionMesh.vertices(hand.palm.root.collisionMesh.vertices(:,3)>0,3));
range(3,2) = max(hand.palm.root.collisionMesh.vertices(:,3)) + 2;
%   Rotation;
%   X
range(4,1) = -2;
range(4,2) = 2;
