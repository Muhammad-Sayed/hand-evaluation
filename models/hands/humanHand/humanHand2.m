...
% scale pose ( m => mm ), angles in rads
...
hand.palm.joints(:,:,1)             = transl(CMC1a.pose(1) * 1000, CMC1a.pose(2) * 1000, CMC1a.pose(3) * 1000) * trotx(CMC1a.pose(4)) * troty(CMC1a.pose(5)) * trotz(CMC1a.pose(6));
...
hand.palm.joints(:,:,2)             = transl(CMC1b.pose(1) * 1000, CMC1b.pose(2) * 1000, CMC1b.pose(3) * 1000) * trotx(CMC1b.pose(4)) * troty(CMC1b.pose(5)) * trotz(CMC1b.pose(6));
% MC1.parent_body = 'CMC1b';
MC1.pose                            = [ 0.03354848  -0.01248297 0.01216455  -2.86224591 0.85165352  1.94658162 ];
hand.palm.links(1).pose             = transl(MC1.pose(1) * 1000, MC1.pose(2) * 1000, MC1.pose(3) * 1000) * trotx(MC1.pose(4)) * troty(MC1.pose(5)) * trotz(MC1.pose(6));
hand.palm.links(1).parent           = 2;
...
hand.palm.root.collisionMesh        = meshes.bone2_1;
% No link transform for this segment
%IMC3.parent_body = 'ground';
IMC3.pose                           = [ 0.00027300  0.01967667  -0.00381503 1.77115361  -0.08468856 0.10358527 ];
hand.palm.jointParent(3)            = 0;
hand.palm.joints(:,:,3)             = transl(IMC3.pose(1) * 1000, IMC3.pose(2) * 1000, IMC3.pose(3) * 1000) * trotx(IMC3.pose(4)) * troty(IMC3.pose(5)) * trotz(IMC3.pose(6));
%MC3.parent_body = 'IMC3';
MC3.pose                            = [ -0.00495342 -0.00967755 0.01997446  -1.55662924 0.07360124  0.02460249 ];
hand.palm.links(2).pose             = transl(MC3.pose(1) * 1000,MC3.pose(2) * 1000,MC3.pose(3) * 1000) * trotx(MC3.pose(4)) * troty(MC3.pose(5)) * trotz(MC3.pose(6));
hand.palm.links(2).parent           = 3;
hand.palm.links(2).collisionMesh    = meshes.bone3_1;
% Apply link transform
...
hand.palm.jointParent(4)            = 3; % this is parent joint, but this joint is originally defined w.r.t MC3 bone segment
hand.palm.joints(:,:,4)             = hand.palm.links(2).pose * transl(IMC4.pose(1) * 1000,IMC4.pose(2) * 1000,IMC4.pose(3) * 1000) * trotx(IMC4.pose(4)) * troty(IMC4.pose(5)) * trotz(IMC4.pose(6));
%MC4.parent_body = 'IMC4';
MC4.pose                            = [ 0.00303550  -0.01287090 0.02243187 -1.14607211  0.08117748  0.23798861 ];
hand.palm.links(3).pose             = transl(MC4.pose(1) * 1000,MC4.pose(2) * 1000,MC4.pose(3) * 1000) * trotx(MC4.pose(4)) * troty(MC4.pose(5)) * trotz(MC4.pose(6));
...
hand.palm.joints(:,:,5)             = hand.palm.links(3).pose * transl(IMC5.pose(1) * 1000,IMC5.pose(2) * 1000,IMC5.pose(3) * 1000) * trotx(IMC5.pose(4)) * troty(IMC5.pose(5)) * trotz(IMC5.pose(6));
%MC5.parent_body = 'IMC5';
MC5.pose                            = [ 0.00258619  -0.01433440 0.01765157  -1.28525283 0.01422445  0.05413693 ];
hand.palm.links(4).pose             = transl(MC5.pose(1) * 1000,MC5.pose(2) * 1000,MC5.pose(3) * 1000) * trotx(MC5.pose(4)) * troty(MC5.pose(5)) * trotz(MC5.pose(6));
...
hand.digits(1).base                     = 2; % this is parent palm joint, but this joint is originally defined w.r.t MC1 bone segment
...
hand.digits(3).base                     = 3; % this is parent palm joint, but this joint is originally defined w.r.t MC3 bone segment
hand.digits(3).joints(:,:,1)            = hand.palm.links(2).pose * transl(MCP3a.pose(1) * 1000,MCP3a.pose(2) * 1000,MCP3a.pose(3) * 1000) * trotx(MCP3a.pose(4)) * troty(MCP3a.pose(5)) * trotz(MCP3a.pose(6));
...
% % Apply alignment transform to bones (and joints)
% % Define the alignment transform used to fix the wrist pose and difference between bone and skin meshes
% hand.aT = trotz(deg2rad(-10)) * transl(40, -10, 0) * trotz(deg2rad(90)) * troty(deg2rad(-90));
% hand.aT = trotx(deg2rad(90)) * trotz(deg2rad(90));
% R = [ hand.aT(:,1:3) [0; 0; 0; 1] ];
% hand.palm.root.collisionMesh.vertices           = (h2e( hand.aT * e2h(hand.palm.root.collisionMesh.vertices.') ))';
% hand.palm.root.collisionMesh.verticesNormals    = (h2e( R * e2h(hand.palm.root.collisionMesh.verticesNormals.') ))';
% hand.palm.root.collisionMesh.normals            = (h2e( R * e2h(hand.palm.root.collisionMesh.normals.') ))';
% % joint axes
% % for j = 1:5
% %     hand.palm.jointAxes(j,:)                    = (h2e( R * e2h(hand.palm.jointAxes(j,:)') ))';
% % end
% % links
% for j = 1:4 
%     hand.palm.links(j).collisionMesh.vertices           = (h2e( hand.aT * e2h(hand.palm.links(j).collisionMesh.vertices.') ))';
%     hand.palm.links(j).collisionMesh.verticesNormals    = (h2e( R * e2h(hand.palm.links(j).collisionMesh.verticesNormals.') ))';
%     hand.palm.links(j).collisionMesh.normals            = (h2e( R * e2h(hand.palm.links(j).collisionMesh.normals.') ))';
% end
% for i = 1:5
%     if i == 1
%         m = 2;
...
%         hand.digits(i).links(j).collisionMesh.normals           = (h2e( R * e2h(hand.digits(i).links(j).collisionMesh.normals.') ))';
%         % Don't forget joint locations and axes
%         
%     end
% end
% % Joints
% for i = 1:5
%     hand.palm.joints(:,:,i) = hand.aT * hand.palm.joints(:,:,i);
% end
% for i = 1:5
%     [~, ~, m] = size(hand.digits(i).joints); % get number of joints in the digit
%     for j = 1:m
%         hand.digits(i).joints(:,:,j) = hand.aT *  hand.digits(i).joints(:,:,j);
%     end
% end
...
plotFrameOrigin( eye(4,4), 20 ); % TEMPRORAY
plotFrameOrigin( eye(4,4), 20 ); % TEMPRORAY
plotFrameOrigin( eye(4,4), 20 ); % TEMPRORAY
...
Tp = zeros(4,4,5); % preallocate variables
Rp = zeros(4,4,5);
for i = 1:5
    if hand.palm.jointParent(i) > 0
        n = hand.palm.jointParent(i);
        Tp(:,:,i) = Tp(:,:,n) * hand.palm.joints(:,:,i) * trotz(hand.qHome{1}(i));
    else
        Tp(:,:,i) = hand.palm.joints(:,:,i) * trotz(hand.qHome{1}(i));
    end
    Rp(:,:,i) = [ Tp(:,1:3,i) [0; 0; 0; 1] ];
    plotFrameOrigin( Tp(:,:,i), 20 ); % TEMPRORAY
end

% Apply transform to all links
for i = 1:4
    n = hand.palm.links(i).parent;
    hand.palm.links(i).collisionMesh.vertices           = (h2e( Tp(:,:,n) * e2h(hand.palm.links(i).collisionMesh.vertices.') ))';
...
for i = 1:5
    % Calculate transform for all joints
    [~, ~, m] = size(hand.digits(i).joints); % get number of joints in the digit
    Td{i} = zeros(4,4,m); % preallocate variables
    Rd{i} = zeros(4,4,m);
    % Variable to track joint location for use in posture analysis
    hand.digits(i).jointLocation = zeros(3,m);
    % first solve the first joint
    j = 1;
    if hand.digits(i).base > 0
        Td{i}(:,:,j) = Tp(:,:,hand.digits(i).base);
    else
        Td{i}(:,:,j) = eye(4,4);
    end
    Td{i}(:,:,j) = Td{i}(:,:,j) * hand.digits(i).joints(:,:,j) * trotz(hand.qHome{i+1}(j));
    Rd{i}(:,:,j) = [ Td{i}(:,1:3,j) [0; 0; 0; 1] ];
    plotFrameOrigin( Td{i}(:,:,j), 15 ); % TEMPRORAY
    % Keep track of joint location for use with posture analysis
    %hand.digits(i).jointLocation(3,j) = Td{i}(1:3,4,j);
    % then loop through the remaining joints
    for j = 2:m
        Td{i}(:,:,j) = Td{i}(:,:,j-1) * hand.digits(i).joints(:,:,j) * trotz(hand.qHome{i+1}(j));
        Rd{i}(:,:,j) = [ Td{i}(:,1:3,j) [0; 0; 0; 1] ];
        plotFrameOrigin( Td{i}(:,:,j), 15 ); % TEMPRORAY
        % Keep track of joint location for use with posture analysis
        %hand.digits(i).jointLocation(3,j) = Td{i}(1:3,4,j);
    ...
    for j = 1:m
        ...
        hand.digits(i).links(j).collisionMesh.vertices          = (h2e( Td{i}(:,:,n) * e2h(hand.digits(i).links(j).collisionMesh.vertices.') ))';
...
% Add flags to indicate presence of alignment transform
% hand.aTflags =  true; 
