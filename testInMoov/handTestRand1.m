function [ Qbest, qObjectBest, qHandBest] = handTestRand1( hand, object, objectRange, anthroReference, attempts )
...
while i < attempts+aborted+timedout+nocontact
    %fprintf('Starting attempt %d ... checking collision ... ', i);
    qObject = randomPose(objectRange, 0.5, 1);
    ...
    if handObjectCollision( handPose, objectPose )
        ...
        [ qHand, TimeOutFlag] = InMoovCloseHandFingersFirst( hand, objectPose, 30 );
        if TimeOutFlag
            %fprintf('TimeOut\n')
            timedout = timedout + 1;
        else
